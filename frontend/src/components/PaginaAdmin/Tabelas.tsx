...
export default function Tabelas({ usersAll, usersActions }) {
  ...
  const numUsuariosAtivos = Array.isArray(usersAll) ? usersAll.filter((item) => item.usuario_status_registro === 'Ativo').length : 0;
  const numUsuariosInativos = Array.isArray(usersAll) ? usersAll.filter((item) => item.usuario_status_registro === 'Inativo').length : 0;
  ...
  async function inativar(item) {
    const response = await fetch(`http://localhost:3001/inativarUsuario/${item.usuario_nome}`, {
      method: "DELETE",
    })
            ...
            <tbody>
              {recordsUsers &&
                ...
                recordsUsers.map((item, i) => (
                  <tr key={i}>
                    <td>
                      <span className={item.usuario_status_registro === 'Ativo' ? styles.situacaoOn : styles.situacaoOff}>
                        <input type="button" />
                      </span>
                    ...
                    <td>
                      ...
                      {IsOpenPopupADM && <PopupEdicaoADM setIsOpenPopup={setIsOpenPopupADM} />}
                    </td>
                    <td>{item.to_char}</td>
